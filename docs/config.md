...
| `hejje.ratings.surveillance.enabled` | — | `true` (`false` in `test`) | Fetch NSE's ASM/GSM lists with the evening D1 refresh (only while `hejje.ratings.enabled`); display only (`docs/ratings.md`, "Surveillance"). |
| `hejje.ratings.surveillance.asm-url`, `gsm-url` | — | `https://www.nseindia.com/api/reportASM`, `.../api/reportGSM` | NSE's JSON reports. |
