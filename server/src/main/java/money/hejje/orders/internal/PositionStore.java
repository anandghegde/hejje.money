...
public class PositionStore {
    ...
    public void upsert(Position p) {
        jdbc.sql("""
                INSERT INTO position (id, mode, instrument_id, product, strategy_id, net_quantity, average_price, realized_pnl_paise,
                    fees_paise, day_buy_qty, day_sell_qty, opened_at, updated_at)
                VALUES (:id, :mode, :instr, :product, :strategy, :net, :avg, :realized, :fees, :dayBuy, :daySell, :openedAt, :updatedAt)
                ON CONFLICT (mode, instrument_id, product, strategy_id) DO UPDATE SET net_quantity = EXCLUDED.net_quantity,
                    ...
                    fees_paise = EXCLUDED.fees_paise, day_buy_qty = EXCLUDED.day_buy_qty, day_sell_qty = EXCLUDED.day_sell_qty,
                ...
                .param("avg", p.averagePrice()).param("realized", p.realizedPnl().paise()).param("fees", p.fees().paise()).param("dayBuy", p.dayBuyQty())
    ...
    private Position map(ResultSet rs, int i) throws SQLException {
        ...
        return new Position(
                ...
                Money.ofPaise(rs.getLong("fees_paise")), rs.getInt("day_buy_qty"), rs.getInt("day_sell_qty"),
